...
def _process_group(group, resume_jobs: List, group_idx: int) -> Dict:
    requirement_scores = []
    match_details = []
...
def _find_best_match(
    ...
    for job in resume_jobs:
        combined_keywords = _build_keywords(job)
        if not combined_keywords:
            continue

        similarity = _calculate_keyword_similarity(experience_area, combined_keywords)
    ...
    year_score  = _calculate_year_score(total_years, min_years, max_years)
...
def _calculate_keyword_similarity(
        experience_area: str, keywords: List[str]
    ) -> float:
        if not keywords:
            return 0.0
        req_vec      = embedder.embed_texts([experience_area])[0]
        keyword_vecs = embedder.embed_texts(keywords)
        return float(max(
            embedder.cosine_similarity(req_vec, kw_vec)
...
def _build_keywords(job) -> List[str]:
    """
