...
class ResumeService:
    ...
    # ============================================================
    # OPTION 2: LOCAL-BASED (upload files via API, no S3/Docker)
    # Route: POST /upload-resumes-local/{job_id}
    # ============================================================
    def upload_resumes_for_job_local(self, job_id: str, files: list[UploadFile]) -> dict:
        """
        Local flow (no S3, no Docker):
        - Accepts PDF uploads directly via API
        - Saves PDFs locally
        - Parses each PDF
        - Saves JSON locally
        ...
        for index, file in enumerate(files, start=1):
            ...
            error_msg = ""
            resume_obj = None
            local_pdf_path = resumes_folder / file.filename
            ...
            try:
                ...
                # ── Parse resume ───────────────────────────────────────────
                resume_text = extract_text_from_pdf(str(local_pdf_path))
                resume_obj = self.parse(resume_text)

            except Exception as e:
                logger.error({
                    "event": "resume_local_parse_error",
                    "file": file.filename,
                    "error": str(e)
                })
                parse_failed = True
            ...
            if not parse_failed and resume_obj is not None:
                ...
                resume_json_path = resumes_folder / f"{Path(file.filename).stem}.json"
                ...
                with open(details_path, "a") as f:
                    f.write(f"\n{file.filename} Parsing Time : {fmt(resume_start)} to {fmt(resume_end)}\n")

                results.append({
                    "index": index,
                    "filename": file.filename,
                    "status": "success",
                    "saved_pdf": str(local_pdf_path),
                    "saved_json": str(resume_json_path)
                })
            else:
                with open(details_path, "a") as f:
                    f.write(f"\n{file.filename} Parsing Time : {fmt(resume_start)} to {fmt(resume_end)} [FAILED]\n")
